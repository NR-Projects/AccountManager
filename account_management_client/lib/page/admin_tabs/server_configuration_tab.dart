...
class _ServerConfigurationTabState extends State<ServerConfigurationTab> {
  ServerInfo serverConfig = ServerInfo.emptyConfig();
  ...
  Future<void> initializeData() async {
    final fetchedServerConfig =
        await widget.serverInfoService.getServerConfig();
    setState(() {
        ...
        child: Column(
          ...
          children: [
            ...
            _buildConfigurationRow(
              ...
              serverConfig.userDeviceAccessState ? "Yes" : "No",
            ),
            _buildConfigurationRow(
              "Last Updated",
              "${serverConfig.lastModifiedDate.toLocal()}".split(' ')[0],
            ),
          ],
        ),
      ),
    ...
    return Column(
      children: [
        ListTile(
          ...
          trailing: Switch(
            value: serverConfig.userDeviceAccessState,
  ...
  void _showChangeMasterPasswordDialogUI(BuildContext context) {
    ...
    Future<void> submitUpdatedMasterPassword() async {
      await widget.serverInfoService.changeMasterPassword(masterPasswordController.text);
  ...
  Future<void> submitGuestRequestAccessChange(bool value) async {
    setState(() {
      serverConfig.userDeviceAccessState = value;
    });

    if (value) {
      await widget.serverInfoService.enableGuestRequestState();
    } else {
      await widget.serverInfoService.disableGuestRequestState();
    }
  }

  Future<void> submitRequestToAddRegisterSecret() async {
    await widget.serverInfoService.addNewRegisterSecret();
  }
